...
'''PyMOL plugin that provides list_contacts command and GUI 
...
def list_contacts(selection,selection2,result="contacts",cutoff=3.6, bigcutoff = 4.0, SC_DEBUG = DEBUG):
    """
    USAGE
    
    list_contacts selection, selection2, [result=contacts],[cutoff=3.6],[bigcutoff=4.0]
    ...
    if onlyacceptors1_sele_count and onlyacceptors2_sele_count:
        ...
        if aa_dist_out < 0:
            print('\n\nCaught a pymol selection error in acceptor-acceptor selection of list_contacts')
            print('accres:', accres)
    ...
    if onlydonors1_sele_count and onlydonors2_sele_count:
        ...
        if dd_dist_out < 0:
            print('\n\nCaught a pymol selection error in dd selection of list_contacts')
            print('donres:', donres)
    ...
    #if the list_contacts debug level is high enough, don't delete them.
    if SC_DEBUG < 5:
    ...
    return True
cmd.extend('contacts', list_contacts) #contacts to avoid clashing with cluster_mols version
...
about_text = '''list_contacts was factored out of the much more full-featured cluster_mols
...
class list_contacts:
    ...
    def button_pressed(self, result):
        if hasattr(result,'keycode'):
            ...
            list_contacts(s1,s2,'%s_%s'%(s1,s2))
            self.select_dialog.withdraw()            
...
def __init__(self):
    self.menuBar.addmenuitem('Plugin', 'command', 'list contacts', label = 'list contacts', command = lambda s=self : list_contacts(s))  
